...
async function getJobsFromNaukri(browserInstance, webPageLink, jobTitle) {
    try {
        ...
        for (let i = 0; i < firstPageJobs.length; i++) {
            RemJobs.unshift(firstPageJobs[i]);
        }
...
async function getJobsFromFreshersWorld(browserInstance, webPageLink, jobTitle) {
    try {
        ...
        await Page.waitForSelector(".srp-pagination.clearfix", { visible: true });
        let PageLinksArr = await Page.evaluate(consoleFnforFreshersWorld, ".srp-pagination.clearfix em a");
        ...
        let firstPageJobs = await Page.evaluate(GettingJobDetailsOfFresherworks, ".clearfix.job-bx.wht-shd-bx h2 a", ".joblist-comp-name",
            ".top-jd-dtl.clearfix li");

        //console.log(firstPageJobs);

        let RemJobs = await RemJobsOfFresherworks(Page, PageLinksArr);
...
async function RemJobsOfFresherworks(Page, PageLinksArr) {
    try {
        let RemJobsArr = [];
        for (let i = 0; i < PageLinksArr.length; i++) {
            await Page.goto(PageLinksArr[i]);
            await Page.waitForSelector(".clearfix.job-bx.wht-shd-bx h2", { visible: true });
            await Page.waitForSelector(".joblist-comp-name", { visible: true });
            await Page.waitForSelector(".top-jd-dtl.clearfix li", { visible: true });

            let EveryPageJobs = await Page.evaluate(GettingJobDetailsOfFresherworks, ".clearfix.job-bx.wht-shd-bx h2 a", ".joblist-comp-name",
            ".top-jd-dtl.clearfix li");

           // console.log("lenght of each page array-"+EveryPageJobs.length);

            for (let j = 0; j < EveryPageJobs.length; j++) {
                RemJobsArr.push(EveryPageJobs[j]);
            }
...
function consoleFnforFreshersWorld(getPageArr) {
    let arr = document.querySelectorAll(getPageArr);
    let pageLinks = [];
    for (let i = 0; i < arr.length; i++) {
        pageLinks.push(arr[i].href);
...
function GettingJobDetailsOfFresherworks(LinkAndName, CompanyName, Exp) {
    let jobDetails = [];
    let ln = document.querySelectorAll(LinkAndName);
    let cn = document.querySelectorAll(CompanyName);
